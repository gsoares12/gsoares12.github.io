# 👋 Olá! Eu sou o Guilherme Soares

## Quem sou eu? 

Analista de Infraestrutura em transição para Engenharia de Dados, com sólida base em
sistemas, redes e automação. Experiência prática com Python, bancos de dados
relacionais (PostgreSQL), BigQuery e processos de ETL. Familiaridade com ambientes
Linux, containers Docker e orquestração de tarefas com Apache Airflow. Perfil analítico,
organizado e orientado à melhoria contínua de processos.

## 🎯 Objetivo:
​
Atuar na construção e manutenção de pipelines de dados, integração entre sistemas,
automação de processos e apoio à tomada de decisão por meio de dados confiáveis e
escaláveis. 

📚  **Atualmente estudando...** 

- Python, SQL, ETL, Cloud, 
- Plataforma Cursos Complementares: [Alura](https://cursos.alura.com.br/user/guilhermehenrique1954-gh)
---

### Tecnologias e linguagens
[![My Skills](https://skillicons.dev/icons?i=gcp,py,anaconda,bash)](https://skillicons.dev)

### Ferramentas de desenvolvimento
[![My Skills](https://skillicons.dev/icons?i=git,github,gitlab,vscode,postman,linux,ubuntu,debian,docker)](https://skillicons.dev)

### Banco de Dados
[![My Skills](https://skillicons.dev/icons?i=sqlite,postgres,firebase,mongodb)](https://skillicons.dev)

---

## 📈 Stats do GitHub

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=gsoares12)](https://github.com/anuraghazra/github-readme-stats)
---

## 📫 Contato

- ✉️ Email: gsoaressh@gmail.com.br 
- 🔗 LinkedIn: [Guilherme Soares](https://www.linkedin.com/in/guilherme-henrique-soares-343481197/)
- 📞 Telefone: +55 3199603-5545
---

✨ *Obrigado por visitar meu perfil!*

